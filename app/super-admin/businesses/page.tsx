...
import ActivityTrendChart, {
    type ChartDataPoint,
} from "@/components/ActivityTrendChart";
import BusinessHistoryTable, {
    type BusinessVisit,
} from "@/components/businesses/business-history-table";
...
export default function SuperAdminBusinessesPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const mappedPlaces: BusinessVisit[] = (placesRes.data.data || []).map(
                    (p: any) => ({
                        id: p.id,
                        businessName: p.name,
                        email: p.email || "N/A",
                        ...
                        time: {
                            ...
                            clock: new Date(p.createdAt).toLocaleTimeString(),
                        },
                    })
                );

                const mappedClaims: BusinessVisit[] = (claimsRes.data.data || []).map(
                    (c: any) => ({
                        id: c.id,
                        ...
                        time: {
                            ...
                            clock: new Date(c.createdAt).toLocaleTimeString(),
                        },
                    })
                );
    ...
    const handleApproveClaim = async (claimId: string) => {
        try {
            await apiClient.patch(`/claims/${claimId}/status`, {
                status: "approved",
            });
            ...
            <div className="space-y-8">
                ...
                <Tabs defaultValue="active" className="w-full">
                    ...
                    <TabsContent value="restricted" className="mt-4">
                        <div className="rounded-lg border border-brand-dark-800 p-6 text-brand-dark-300">
                            Restricted view coming soon.
                        </div>
                    </TabsContent>
                </Tabs>
            </div>
